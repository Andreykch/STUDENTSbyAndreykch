...
class StudentsModel {
    ...
    Set (students) {
      ...
      for (const i in this.displyedStudents) {
        console.log(i)
    ...
    compareString (str1, str2) {
      const rx = /([^\d]+|\d+)/ig
      const str1split = str1.match(rx)
      const str2split = str2.match(rx)
      for (let i = 0, l = Math.min(str1split.length, str2split.length); i < l; i++) {
        const s1 = str1split[i]
        const s2 = str2split[i]
        if (s1 === s2) continue
        if (isNaN(+s1) || isNaN(+s2)) { return s1 > s2 ? 1 : -1 } else { return +s1 - s2 }
      }
      return 0
    }
